...
submit <- local({
    ...
    get_courseid <- function() {
        pretty_out("The first item I need is your Course ID. For example, if the",
                   ...
                   "then your course ID would be 'rprog-001'",
                   "(without the quotes).",
                   skip_after=TRUE)
        repeat {
            ...
            if(!any(is_url, is_numbers, is_example)){
                ...
                if(is_url) {
                    pretty_out("It looks like you entered a web address,",
                               "which is not what I'm looking for.")
                }
                ## Check if courseid is all numbers
                if(is_numbers) {
                    pretty_out("It looks like you entered a numeric ID,",
                               "which is not what I'm looking for.")
                }
                ## Check if the user stole the example courseid
                if(is_example) {
                    pretty_out("It looks like you entered the Course ID that I",
                               "used as an example, which is not what I'm",
                               "looking for.")
                }
            }
    ...
    function(manual = FALSE, resetLogin = FALSE) {
        ...
        if(!manual) {
            ...
            ch_ok <- !is(ch, "try-error") &&
                exists("ch.key", ch) &&
                !is.na(ch$ch.key)
            if(!ch_ok) {
                stop("Either the course ID you entered is not valid or your",
                     "course site is unreachable at this time. If you'd like to",                      "submit manually, you can run submit(manual=TRUE).")
            }
